...
export default function App() {
  ...
  const handleAddCar = () => {
    ...
    if (!regNumber.trim() || !brand.trim()) {
      setError("Vänligen fyll i BÅDA fälten för att lägga till en bil!");
      return;
    }

    const regex: RegExp = /^[A-Z]{3}\d{3}$|^[A-Z]{3}\d{2}[A-Z]$/i;
    const correctFormat: boolean = regex.test(regNumber);
    if (!correctFormat) {
      setError("Vänligen ange regnumret i rätt format!");
      return;
    }
    ...
    const newCarsList: ICar[] = [...cars, car];
    setCars(newCarsList);
  };
  ...
  const handleDeleteCar = (id: number) => {
    ...
    const remainingCars: ICar[] = cars.filter((car) => car.id !== id);
